...
case('minseqit')
  read(50,*,err=20) minseqit
  if (minseqit.lt.1) then
    write(*,*)
    write(*,'("Error(readinput): minseqit < 1 : ",I8)') minseqit
    write(*,*)
    stop
  end if
case('maxseqit')
  read(50,*,err=20) maxseqit
  if (maxseqit.lt.1) then
    write(*,*)
    write(*,'("Error(readinput): maxseqit < 1 : ",I8)') maxseqit
    write(*,*)
    stop
  end if
case('bseqit')
  read(50,*,err=20) bseqit
  if (bseqit.le.0.d0) then
    write(*,*)
    write(*,'("Error(readinput): bseqit <= 0 : ",G18.10)') bseqit
    write(*,*)
    stop
  end if
case('epsseqit')
  read(50,*,err=20) epsseqit
  if (epsseqit.lt.0.d0) then
    write(*,*)
    write(*,'("Error(readinput): epsseqit < 0 : ",G18.10)') epsseqit
    write(*,*)
    stop
  end if
case('nseqit')
...
! read tensor component
    read(50,*,err=20) t1,t2
    tmfix(i)=cmplx(t1,t2,8)
! read parity and spatial and spin axis-angle rotations
    read(50,'(A256)',err=20) str
    str=trim(str)//' 0.0 0.0 0.0 0.0'
    read(str,*,err=20) p,v1(:),t1,v2(:),t2
    if (abs(p).ne.1) then
      write(*,*)
...
! covert angles to radians
    t1=t1*pi/180.d0
    t2=t2*pi/180.d0
! compute the spatial and spin 3x3 rotation matrices
    call axangrot(v1,t1,rtmfix(:,:,1,i))
    call axangrot(v2,t2,rtmfix(:,:,2,i))
